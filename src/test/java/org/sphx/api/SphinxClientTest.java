...
public class SphinxClientTest extends TestCase {
    ...
    protected void setUp() throws Exception {
        super.setUp();
        sphinxClient = new SphinxClient("192.168.10.81", 9312);
    }
    ...
    public void testResponseQueryWithNonExistWord() throws SphinxException {
        SphinxResult result = sphinxClient.query("test1", "main");
    ...
    public void testResponseQuery() throws SphinxException {
        ...
        //设定排序
        //sphinxClient.setSortMode(SphinxClient.SPH_SORT_ATTR_DESC,"title");
        //设定分组
        //sphinxClient.setGroupBy("title",SphinxClient.SPH_GROUPBY_ATTR);
        //执行查询
        SphinxResult result = sphinxClient.query("四大名著", "main");
        System.out.println("查询结果条数：" + result.getMatches().size() + " 总共条数:" + result.totalFound);

        for (int i = 0; i < result.words.length; i++) {
            System.out.println("命中查询词语：" + result.words[i].getWord() + " 命中次数：" + result.words[i].getHits());
        }

        List<SphinxMatch> matches = result.getMatches();
        System.out.println("查询内容：" + matches.get(1).getAttribute("title") + "待处理...");
...
//		assertTrue(result.time > -1.0);
        System.out.println(result.getMatches().size() + "total:" + result.totalFound);
        String[] attrNames = result.attrNames;

        System.out.println(result.fields.length);
        for (int i = 0; i < attrNames.length; i++) {
            System.out.println(attrNames[i]);
            switch (result.attrTypes[i]) {
                case SphinxClient.SPH_ATTR_INTEGER:
                case SphinxClient.SPH_ATTR_ORDINAL:
                case SphinxClient.SPH_ATTR_FLOAT:
                default:
                    System.out.print ("遍历：" + matches.get(1).getAttribute(i));
                    break;
            }
        }
...
//		assertEquals(3, sphinxWordInfo.getDocs());
    ...
    public void testResponseQueryForRunQuery() throws SphinxException {
        ...
        int addQuery = sphinxClient.addQuery("linux", "main", "wang");
        assertEquals(0, addQuery);
        ...
        System.out.println("查询结果：" + results.length);
        System.out.println("第一个：" + addQuery + "..." + results[0].getMatches().size());
